<div class="flex flex-col md:flex-row items-center justify-between p-4 md:p-8 min-h-screen mt-[3em] bg-gray-50">
    ...
    <div class="w-full md:w-1/2 md:sticky md:top-20" x-data="{ mainImage: '{{url('storage',$products->images[0])}}' }">
        ...
        <div class="flex items-center mt-6 p-3 bg-white rounded-lg shadow-sm border-l-4 border-purple-500">
            ...
            <div>
                <h3 class="font-bold text-gray-800">@lang('products-detail.free_shipping')</h3>
                <p class="text-sm text-gray-600">@lang('products-detail.delivery_time')</p>
                <p class="text-sm text-gray-600">@lang('products-detail.shipping_spain_only')</p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full md:w-1/2 p-4 md:p-8 mt-6 md:mt-0 bg-white rounded-lg shadow-sm">
        ...
        <span class="inline-block px-3 py-1 text-xs font-semibold text-purple-800 bg-purple-100 rounded-full mb-3">@lang('products-detail.in_stock')</span>
        
        <!-- Nombre traducido con Livewire -->
        <h1 class="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-800">
            @if ($isTranslating && empty($translatedName))
        ...
        <div class="mb-6">
            <h2 class="text-lg font-semibold mb-2 text-gray-800">@lang('products-detail.description')</h2>
            <!-- Descripción traducida con Livewire -->
            <p class="text-gray-600">
                @if ($isTranslating && empty($translatedDescription))
                    <span class="inline-block">
                        <svg class="animate-spin h-5 w-5 text-purple-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                ...
                @endif
                {{ $translatedDescription ?: $products->description }}
            </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mt-4">
            <button wire:click="addToCart({{ $products->id }})" class="w-full sm:w-3/5 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-6 rounded-lg shadow transition duration-200 flex items-center justify-center">
                ...
                @lang('products-detail.add_to_cart')
            </button>
        </div>
        ...
        <div class="mt-8 pt-6 border-t border-gray-200">
            <div class="flex items-center text-sm text-gray-600 mb-2">
                ...
                @lang('products-detail.best_store')
            </div>
            <div class="flex items-center text-sm text-gray-600 mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                @lang('products-detail.return_policy')
            </div>
        </div>
    </div>
</div>